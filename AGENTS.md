...
- **Favicons are now the real logos** (`assets/favicon-pwm.svg`, `prb/assets/favicon-prb.svg`), replacing the generic red circle / blue square data-URIs. Lucas's SVGs are 480×178 wordmarks, so each is centred inside a 480×480 square over its brand colour (`#FF0055` / `#2764CF`). ⚠️ At 32px the wordmark rasterises to **6px tall** — measured, not guessed. It reads as a colour chip, not as a word; the fallback if he wants legibility is the lone `P`. No `?v=` bump: no JS or CSS changed, and the favicon filenames are new.
