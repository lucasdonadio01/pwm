...
- **`js/motor.js` is the shared engine — do not fork it.** Pattern families, seed and morph live there, one instance per figure: the game holds one (`simbolo.js` is a thin facade over it) and the landing mosaic holds one per tile. That is deliberate — the mosaic has to morph exactly like the game, and two copies of that code would drift apart on the first tweak.
- **The entry wipe must stay coarse.** reactbits' Pixel Transition is an 8-cell grid where each cell appears *whole* in 0,4 s; a fine grid of smoothly growing cells reads as a fade and Lucas rejected it twice. `PixelSwap.pantalla` sets a 12-column cell size — do not pass `celda` from the caller, it overrides that.
- **The landing is a mosaic** (`js/mosaico.js`): tiles covering the viewport, each a pueblo's palette and figure, a fifth of them regenerating every 950 ms with a stagger. It leaves a frame so the PixelBlast shows around and between tiles, and `Mosaico.detener()` runs inside the entry wipe so it stops eating frames. Each tile carries a `calor` that rises under the cursor and decays slowly: it scales the tile, draws it above its neighbours (tiles are sorted by `calor` before drawing), holds it out of the regeneration batch, and leaves a trail on a sweep. A click regenerates that tile.
- **Seeded generation.** `mulberry32` + eight pattern families, drawn from a **weighted** menu. **The variety lives inside the families, not in the family draw**: `flor` dispatches to eight species and `mandala` to five traces, and together they take 65 % of the roll (measured over 120 generations). Add variants there, not new top-level families. There is also a fill floor — a variant that lands on three cells is discarded and re-rolled as a flower. **Mirror symmetry is forced centrally** in `generarGrilla` via `espejar()`, not per family — Lucas rejected the lopsided figures the free-growth family produced, and doing it in one place means a family added later cannot reintroduce them. The seed shown under the pueblo name reproduces the symbol exactly and goes in the PNG filename. If you add a family, register it in `FAMILIAS` and it enters the draw automatically.
...
- **Three bugs worth remembering.** (1) `setPointerCapture` ran before painting in `pointerdown`; it throws when the pointer id is not active and swallowed the whole click. (2) The (now removed) GIF encoder's palette sampled every 9th pixel of every frame — half a million samples and median cut took over 40 s. (3) The canvas geometry used for hit-testing is computed inside the draw loop, so a frame drawn while `#taller` was still hidden left it at zero and every click missed; the loop now skips frames where the canvas has no size. In the preview pane this looks like a code bug but is the documented no-rAF-while-hidden gotcha.
