...
- **The landing is a mosaic** (`js/mosaico.js`): tiles covering the viewport, each a pueblo's palette and figure, a fifth of them regenerating every 950 ms with a stagger. It leaves a frame so the PixelBlast shows around and between tiles, and `Mosaico.detener()` runs inside the entry wipe so it stops eating frames.
